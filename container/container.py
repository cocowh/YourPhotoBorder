...
class PhotoBorder(object):
    def __init__(self, path: str, config: dict, source_exif_path: str = "", analyzer: object = None):
        if source_exif_path == "":
        ...
        #     else:
        #         self.analyzer = JpegAnalyzer(path=path,exif_source_path=source_exif_path)
        self.config = config
    ...
    def fix_config(self):
        if self.config['camera_args_direction'] == 'left' or self.config['camera_args_direction'] == 'right':
            if isinstance(self.config['extra_length'], int) is False:
                ...
                self.config['extra_length'] = int(
                    self.analyzer.get_width() * percent2float(self.config['extra_length']))
        if self.config['camera_args_direction'] == 'up' or self.config['camera_args_direction'] == 'down':
            if isinstance(self.config['extra_length'], int) is False:
                # 同上
                self.config['extra_length'] = int(
                    self.analyzer.get_height() * percent2float(self.config['extra_length']))
    ...
    def get_custom_box(self) -> tuple:
        ...
        if self.config['camera_args_direction'] == 'right':
            return (self.analyzer.get_width(), 0, self.config['extra_length'] + self.config['border_size'],
                    self.analyzer.get_height())
        if self.config['camera_args_direction'] == 'up':
            return (0, 0, self.analyzer.get_width(), self.config['extra_length'] + self.config['border_size'])
        if self.config['camera_args_direction'] == 'down':
            return (0, self.analyzer.get_height(), self.analyzer.get_width(),
                    self.config['extra_length'] + self.config['border_size'])
