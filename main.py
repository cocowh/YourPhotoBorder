...
def photo_border_single_worker(input_path, output_path, source_path):
    local_source_path = input_path if (
            source_path == input_path or source_path is None or source_path == "") else source_path
    analyzer = JpegAnalyzer(local_source_path)
    camera_company = analyzer.get_camera_company()
    if camera_company is None:
        camera_company = args.default_image_maker
        analyzer.tags["Image Make"] = camera_company
        analyzer.tags["Image Model"] = camera_company
    target_rule = None
    ...
    print(
        f"[*] Target Rule: {target_rule['camera_company']}\n\tPhotoPath: {input_path}\n\tOutputPath: {output_path}\n\tExifSourcePath: {local_source_path}\n\tConfig: {border_config}\n")
    with open(border_config, "r") as f:
        PhotoBorder(f"{input_path}", json.loads(f.read()), source_exif_path=local_source_path, analyzer=analyzer) \
            .generate() \
            .save(output_path)
...
def main():
    ...
    if os.path.isdir(args.file):
        if not os.path.isdir(args.output):
            ...
            exit(-1)
        if args.source_file is None and os.path.isdir(args.source_file) == False:
